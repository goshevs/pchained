...
pchained (s1_i, include(mean(s2_i s5_i s6_i)) scale omit(x*)) /// 
		 (s2_i, include(mean(s1_i s5_i s6_i)) scale) ///
		 (s5_i, include(mean(s1_i s2_i s6_i)) scale) ///
		 (s6_i, include(mean(s1_i s2_i s5_i)) scale) ///
		 (y2 x5*, noimputed)  ///
		 (y4 i.yx i.yz x5, include(y2 mean(s1_i))) ///
		 (y5 i.yx x1 i.yz x5 i.x2, include(y*)) ///
		 (y6, noimputed omit(x1 x3 y1)), ///
	     i(id) t(time) ///
...
*** Problems: omit needs to remove all variables that are with asterisk from list common regressors!
